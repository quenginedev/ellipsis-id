...
export async function getFontFingerprint(): Promise<ComponentData> {
  return new Promise((resolve) => {
    ...
    try {
      ...
      detectFonts(FontDetectionMethod.SPAN).then((detectedFonts) => {
        ...
        detectFonts(FontDetectionMethod.CANVAS).then((detectedFonts) => {
          clearTimeout(timeoutId);
  ...
  return new Promise((resolve) => {
    ...
    const fontPromises = COMMON_FONTS.map((font) => {
      return new Promise<void>((resolveFontCheck) => {
        ...
        // Get image data
        const testImageData = context.getImageData(0, 0, canvas.width, canvas.height).data;
